...
export function Login() {
  ...
  onClick={() => {
    ...
    if (newShow) {
      // Eye ON: show full real password
      setDisplayPassword(password);
      if (revealTimeoutRef.current !== null) {
        window.clearTimeout(revealTimeoutRef.current);
      }
    } else {
      // Eye OFF: mask all characters again
      setDisplayPassword('•'.repeat(password.length));
    }
  }}
...
>
  ...
  onClick={() => {
    setShowPassword((prev) => {
      ...
      // If eye is on, show full password
      // If off, mask all
      setDisplayPassword(newState ? password : '•'.repeat(password.length));
